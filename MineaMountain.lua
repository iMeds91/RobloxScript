...
local Remotes = {
	...
	RadarShopQuery = findRemote("RadarShopQuery"),
	RadarBuyRequest = findRemote("RadarBuyRequest"),
	RadarShopRestocked = findRemote("RadarShopRestocked"),
...
local Window = Library:CreateWindow({
	...
	Footer = "Mine a Mountain | Update by iMeds",
	AutoShow = true,
	NotifySide = "Right",
...
do
	local function install()
		...
		local function setActive(value)
			...
			-- Deferred so a saved FarmTargets selection (applied via SaveManager
			-- autoload in the same pass, but not necessarily before this toggle)
			-- has a chance to populate `targets` before we validate it.
			task.defer(function()
				local store = Library and Library.Toggles
				local entry = store and store.AutoFarmBoulders

				local function stillWanted()
					if entry and type(entry.Value) == "boolean" and entry.Value ~= true then
						return false
					end
					return true
				end
				...
				local remaining = STABILIZE_SECONDS - (os.clock() - ScriptStartClock)
				if remaining > 0 then
					statusText = string.format("Stabilizing (%.0fs)", remaining)
					task.wait(remaining)
				end

				if startToken ~= token or not stillWanted() then
					return
				end
...
do
	local function install()
		...
		local function buyAllAvailableRadars()
			local query = Remotes.RadarShopQuery
			local buyReq = Remotes.RadarBuyRequest
			...
			for radarId, amount in pairs(data.stock) do
				local count = tonumber(amount) or 0
				if count > 0 then
					for _ = 1, count do
						local success, res = pcall(function()
							return buyReq:InvokeServer(radarId)
						end)
						if success and res and res.ok then
							boughtTotal += 1
						else
		...
		if Remotes.RadarShopRestocked then
			Connections.radarRestockConn = Remotes.RadarShopRestocked.OnClientEvent:Connect(function()
				if State.autoBuyRadars then
					task.spawn(buyAllAvailableRadars)
				end
			end)
		end

		local radarCheckClock = 0
		Connections.radarLoopConn = Services.RunService.Heartbeat:Connect(function(deltaTime)
			if State.autoBuyRadars then
				radarCheckClock += deltaTime
				if radarCheckClock >= 10.0 then
					radarCheckClock = 0
					task.spawn(buyAllAvailableRadars)
				end
			end
		...
		local RadarBox = Tabs.farming:AddLeftGroupbox("Radar Auto Buy", "radar")
		RadarBox:AddLabel("Buys all radars from shop whenever stock is available", true)
		RadarBox:AddDivider()

		RadarBox:AddToggle("AutoBuyRadars", {
			Text = "Auto Buy On Restock",
			Default = Config.AutoBuyRadars,
			Callback = function(value)
				State.autoBuyRadars = value
				if value then
					task.spawn(buyAllAvailableRadars)
				end
			end,
		...
		RadarBox:AddButton("Buy All Stock Now", function()
			task.spawn(function()
				local count = buyAllAvailableRadars()
				if Library then
					Library:Notify(string.format("Purchased %d radars from shop!", count), 3)
				end
			end)
		end)
